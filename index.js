...
const TMDB_API_KEY = "4f7f60c1464bc3126f22bdb955d9192e"
const OPENAI_API_KEY = "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";
...
async function searchTMDB(type, query, lang = 'fa-IR') {
    const url = `https://api.themoviedb.org/3/search/${type}?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}&language=${lang}&include_adult=false`;
    return (await fetch(url)).json();
}
async function discoverByGenre(genreId, lang = 'fa-IR') {
    const url = `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_genres=${genreId}&sort_by=popularity.desc&language=${lang}&include_adult=false`;
    return (await fetch(url)).json();
}
async function findMoviesWithActor(personId, lang = 'fa-IR') {
    const url = `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_cast=${personId}&sort_by=popularity.desc&language=${lang}&include_adult=false`;
    return (await fetch(url)).json();
}
async function findSimilarMovies(movieId, lang = 'fa-IR') {
    const url = `https://api.themoviedb.org/3/movie/${movieId}/similar?api_key=${TMDB_API_KEY}&language=${lang}&include_adult=false`;
    return (await fetch(url)).json();
}
